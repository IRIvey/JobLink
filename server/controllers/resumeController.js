...
const buildResumeFromProfile = (jobSeeker) => {
  return {
    ...
    certifications: (jobSeeker.certifications || []).map((cert) => ({
      id: cert._id?.toString() || makeId("cert"),
      name: cert.name || "",
      issuer: cert.issuer || "",
      date: toYM(cert.date),
      expiryDate: "",
...
// Merge rule:
// - resume.personalInfo overrides profile fields ONLY if that field has a value
// - resume sections (experience/education/skills/certs/projects/languages/interests) used ONLY if non-empty
const mergeResume = (profileResume, savedResume = {}) => {
  ...
  merged.personalInfo = {
    ...
    // always keep latest profile photos unless resume explicitly has them
  ...
  // Array sections: use saved if it has data, else profileResume
  merged.experience =
  ...
  merged.skills =
    Array.isArray(savedResume.skills) && savedResume.skills.length > 0
      ? savedResume.skills
      : profileResume.skills;

  merged.certifications =
    Array.isArray(savedResume.certifications) && savedResume.certifications.length > 0
      ? savedResume.certifications
      : profileResume.certifications;

  merged.projects = Array.isArray(savedResume.projects) ? savedResume.projects : profileResume.projects;
  merged.languages = Array.isArray(savedResume.languages) ? savedResume.languages : profileResume.languages;
  merged.interests = Array.isArray(savedResume.interests) ? savedResume.interests : profileResume.interests;
...
// Get user's resume (ALWAYS dynamic from profile + optional overrides)
export const getResume = async (req, res) => {
  try {
    const jobSeeker = await JobSeeker.findById(req.user.id);
...
// Update entire resume (this stores overrides)
export const updateResume = async (req, res) => {
  try {
    const { resumeData } = req.body;
    ...
    // Return merged so frontend sees latest profile + overrides
    const profileResume = buildResumeFromProfile(jobSeeker);
...
// Update personal info overrides
export const updatePersonalInfo = async (req, res) => {
  try {
    const { personalInfo } = req.body;
...
// -----------------------------
// OPTIONAL: Keep these endpoints
// (they now modify resume overrides, not profile)
// If you want ALL edits to be from Profile only,
// you can remove these endpoints later.
// -----------------------------
...
export const addCertification = async (req, res) => {
  try {
    const { certification } = req.body;
    if (!certification) return res.status(400).json({ message: "Certification data is required" });
...
// Export resume PDF (now exports merged resume so profile changes show in PDF too)
export const exportResume = async (req, res) => {
  try {
    const jobSeeker = await JobSeeker.findById(req.user.id);
    if (!jobSeeker) return res.status(404).json({ message: "User not found" });
    ...
    if (Array.isArray(resume.skills) && resume.skills.length > 0) {
      ...
      doc.fontSize(10).font("Helvetica").fillColor("#000000")
        .text(resume.skills.join(", "), { align: "justify" });
